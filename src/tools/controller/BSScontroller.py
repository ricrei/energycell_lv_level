...
class BSScontroller:
  ...
  def __init__(self, grid, control):
      ...
      if self.set_bss == True:
        ...
        self.P_controller = BSS_control_no_bss(grid, self.intervall_in_seconds, self.busses_num, self.timedelta_charging_delay, self.timedelta_charging_delay_winter, self.efficiency_charge, self.efficiency_discharge)
...
# Parent
class BSS_control:
  def __init__(self, grid, intervall_in_seconds, bss_num, timedelta_charging_delay, timedelta_charging_delay_winter, efficiency_charge, efficiency_discharge): 
      self.intervall = intervall_in_seconds#self.intervall_in_seconds #?????? neu 20.10
      #self.intervall_in_seconds = self.intervall_in_seconds #?????? neu 20.10
  ...
  '''
  def linear_charge(self, grid, t, p_mw_bss, get_e_mwh, free_capacity, timedelta_charging_delay, timedelta_charging_delay_winter, t_start_linear_discharge):
      print('direct charge')
      print('p_mw_bss_dir: ' + str(p_mw_bss[0]))
      print('Lin charge')
      #efficiency_discharge = grid.net.storage.efficiency_storage * grid.net.storage.efficiency_inverter
      ...
      sunrise_next, sunset_next, timedelta_day_next_s, timedelta_sunrise_sunset_next_s = grid.get_timedelta(t + datetime.timedelta(days = 1))
      t_start_linear_charge = sunrise + datetime.timedelta(hours = timedelta_charging_delay)
      #t_start_linear_charge_winter = sunrise + datetime.timedelta(hours = timedelta_charging_delay)
      t_end_linear_charge = sunset - datetime.timedelta(hours = timedelta_charging_delay)
      ...
      if timedelta_sunrise_sunset_s < (12*3600):  #winter
          ...
          if time <= sunrise or time >= t_start_linear_discharge: # bisher: time >=sunset:   
              ...
              if timedelta_night_s < 3600:
                  print('Beeilung')
                  timedelta_night_s = 3600
              
              #p_mw_lin_dch = -(get_e_mwh * 3600)/ timedelta_night_s
              p_mw_lin_dch = -((get_e_mwh - 0 * grid.net.storage.max_e_mwh) * self.efficiency_discharge * 3600)/ timedelta_night_s
              ...
              case_lin_dch2 = (p_mw_bss < 0) & (get_e_mwh < 0.2 * grid.net.storage.max_e_mwh)
      ...
      print('p_mw_bss_lin: ' + str(p_mw_bss[0]))
      return p_mw_bss
#---------------
  def linear_charge_test(self, grid, t, p_mw_bss, get_e_mwh, free_capacity, timedelta_charging_delay, t_start_linear_discharge):
      
      get_soc=self.state_of_charge(grid) #neu
      print('soc:' + str(get_soc))
      ...
      print('time: ' + str(time))
      #print('t: ' + str(t))
      print('start lin discharge: ' + str(t_start_linear_discharge))
      # Linear charging and discharging:
      if timedelta_sunrise_sunset_s < (12*3600):  #winter
          #p_mw_bss =np.ones(len(grid.net.storage))*5
          p_mw_lin_ch = (free_capacity * 3600)/ (timedelta_lin_ch_winter_s * self.efficiency_charge) #timedelta_day_s # efficiency     
          if time == sunrise:
              print('time=sunrise')
          if time == t_start_linear_discharge:
              print('time=t_start_linear_discharge')
          if time <= sunrise or time >= t_start_linear_discharge: # bisher: time >=sunset:  
              print('Nacht')
          #if time <= sunrise or time >= sunset:
              #if (sunset - time).total_seconds() > 0: # nach Mitternacht
              ...
              #p_mw_lin_dch = -(get_e_mwh * 3600)/ timedelta_night_s
              #if timedelta_night_s < 3600: #self.intervall:
                  #print('Vorsicht')
                  #timedelta_night_s = 3600#self.intervall
              p_mw_bss = -np.ones(len(grid.net.storage))*0.012
              #*#p_mw_lin_dch = -((get_e_mwh - 0.2 * grid.net.storage.max_e_mwh) * self.efficiency_discharge * 3600)/ timedelta_night_s
              #p_mw_lin_dch = -(get_e_mwh * self.efficiency_discharge * 3600)/ timedelta_night_s
              ##case_less = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) & (get_e_mwh < 0.2 * grid.net.storage.max_e_mwh)
              ##case_lin_dch = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) & (get_e_mwh >= 0.2 * grid.net.storage.max_e_mwh)
              #case_less = (p_mw_lin_dch >= p_mw_bss) & (p_mw_bss < 0) & (get_soc < 0.2) # >= ?
              #case_less = (p_mw_bss < 0) & (get_soc < 0.2) # >= ?
              #case_lin_dch = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) & (get_soc >= 0.2 )
              #print(case_lin_dch)
              #case_lin_dch = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) #<0 wichtig für Rechenfehler
              #print('p_mw_lin_dch: ' + str(p_mw_lin_dch))
              ##p_mw_bss[case_lin_dch] = p_mw_lin_dch[case_lin_dch]  
              ##p_mw_bss[case_less] = 0
              #if timedelta_night_s < 3600:
                  #print('timedelta_night_s < 3600')
                 # p_mw_bss[p_mw_bss <0]=0 
              #print('case lin less: ' + str(case_less))
              #print('p_mw_bss end: ' + str(p_mw_bss))
      else: # summer
          if time >= t_start_linear_charge and time <= t_end_linear_charge: 
              #p_mw_lin_ch = (free_capacity * 3600)/ timedelta_lin_ch_summer_s
              p_mw_lin_ch = (free_capacity * 3600)/ (timedelta_lin_ch_summer_s * self.efficiency_charge)
...
#----------------  
  def linear_charge_aktuell(self, grid, t, p_mw_bss, get_e_mwh, free_capacity, timedelta_charging_delay, t_start_linear_discharge):
      soc_rest = 0.2
      get_soc=self.state_of_charge(grid) #neu
      print('soc:' + str(get_soc))
      ...
      if timedelta_sunrise_sunset_s < (12*3600):  #winter
          ...
          if time <= sunrise or time >= t_start_linear_discharge: # bisher: time >=sunset:  
              ...
              #p_mw_lin_dch = -(get_e_mwh * 3600)/ timedelta_night_s
                  ...
                  #print('Vorsicht')
                  #timedelta_night_s = 3600#self.intervall
              p_mw_lin_dch = -((get_e_mwh - soc_rest * grid.net.storage.max_e_mwh) * self.efficiency_discharge * 3600)/ timedelta_night_s
              #p_mw_lin_dch = -(get_e_mwh * self.efficiency_discharge * 3600)/ timedelta_night_s
              ##case_less = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) & (get_e_mwh < 0.2 * grid.net.storage.max_e_mwh)
              ##case_lin_dch = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) & (get_e_mwh >= 0.2 * grid.net.storage.max_e_mwh)
              #case_less = (p_mw_lin_dch >= p_mw_bss) & (p_mw_bss < 0) & (get_soc < 0.2) # >= ?
              ##case_less = (p_mw_bss < 0) & (get_soc < soc_rest) # >= ?
              case_lin_dch = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) & (get_soc >= soc_rest )
              #print(case_lin_dch)
              #case_lin_dch = (p_mw_lin_dch > p_mw_bss) & (p_mw_bss < 0) #<0 wichtig für Rechenfehler
              #print('p_mw_lin_dch: ' + str(p_mw_lin_dch))
              p_mw_bss[case_lin_dch] = p_mw_lin_dch[case_lin_dch]  
...
class BSS_control_no_bss(BSS_control):
  def __init__(self, grid, intervall_in_seconds, busses_num, timedelta_charging_delay, timedelta_charging_delay_winter, efficiency_charge, efficiency_discharge):
      super().__init__(grid, intervall_in_seconds, busses_num, timedelta_charging_delay, timedelta_charging_delay_winter, efficiency_charge, efficiency_discharge)

### DIRECT ###
class BSS_control_direct(BSS_control): #simple
  def __init__(self, grid, intervall_in_seconds, bss_num, timedelta_charging_delay, timedelta_charging_delay_winter, efficiency_charge, efficiency_discharge): 
      super().__init__(grid, intervall_in_seconds, bss_num, timedelta_charging_delay, timedelta_charging_delay_winter, efficiency_charge, efficiency_discharge)
...
class BSS_P_control_grid_fid(BSS_control):
  ...
  def pcontrol_linear_charge(self, grid, t): #pcontrol_linear_charge_cbss
      ...
      #------------------
      bss = grid.net.storage
      #p_mw_bss_total_start = - self.residual_load_per_bus(grid)
      #p_mw_bss_total = p_mw_bss_total_start.sum()
      s_res, p_res = grid.get_residualload_s_sum()
      p_mw_bss_total = - p_res
      print('p_mw_bss_total start: ' + str(p_mw_bss_total))
     ...
     # Current parameters of BSS:             
      ...
      if p_mw_bss_total >= 0: # 0 muss auch noch abgefangen werden!!!
          print('Solarüberschuss')
          p_mw_bss = (p_mw_bss_total * distribution_factor_ch) #* np.ones(bss_num)
      
      elif p_mw_bss_total < 0:
          p_mw_bss = (p_mw_bss_total * distribution_factor_dch) #* np.ones(bss_num)
          print('Lastüberschuss')
      # needed capacity:
      #needed_capacity = p_mw_bss * self.intervall / 3600 #
      
      print('p_mw_bss_total distributed: ' + str(p_mw_bss.sum()))
      
      
      #DIRECT CHARGING:
      p_mw_bss = self.direct_charge(grid, p_mw_bss)   
      #print('p direc: ' + str(p_mw_bss[0]))
      #p_mw_bss = self.direct_charge_cbss(grid, p_mw_bss)  
      
      #LINEAR CHARGING AND DISCHARGING:
      sunrise, sunset, timedelta_day_s, timedelta_sunrise_sunset_s = grid.get_timedelta(t)    
      t_start_linear_discharge = sunset - datetime.timedelta(hours = self.timedelta_charging_delay)
      p_mw_bss = self.linear_charge(grid, t, p_mw_bss, get_e_mwh, free_capacity, self.timedelta_charging_delay, self.timedelta_charging_delay_winter, t_start_linear_discharge)
  ...
  def pcontrol_trafo_charge(self, grid, t): #pcontrol(self, grid, t): #t
      ...
      if grid.s_trafo_power < -s_res: # Fall Trafoüberlastung bei Solarüberschuss
          print('Trafoüberlastung bei Solarüberschuss')
          ...
          #print('p_trafo_max: '+ str(p_trafo_max))
          p_total_bss = p_res + p_trafo_max
          print('p_total_bss: '+ str(p_total_bss))
          
         
          # damping:
         # print('get_e_mwh vor damping: ' + str(get_e_mwh))
          get_e_mwh[get_e_mwh <0] = 0
          #print('get_e_mwh nach damping: ' + str(get_e_mwh))
          ...
          #print('damping factor: '+ str(damping_factor[0]))
          '''    
          if free_capacity.sum() == 0.0:
              damping_faktor = np.zeros(len(bss)) # np.zeros[len()]
          else:
          ...
          #print('p_mw_damped 1: ' + str(p_mw_damped[0]))
          #print('soc: ' + str(get_soc))
         
          needed_capacity = - p_mw_damped * self.intervall / 3600 # oder needed_capacity = p_mw_damped + p_mw_bss / ... ? nein
          #print('needed_capacity: ' + str(needed_capacity[0]))
          #print('get_e_mwh: ' + str(get_e_mwh[0]))
          ...
          load_test = (get_soc <= 0)
          #print('load_excess fid 2:' + str(load_excess_fid_2[0]))
          #print('load_test:' + str(load_test[0]))
      ...
      #---------------neu----------------------------------
      p_mw_bss = p_mw_bss + p_mw_damped
      print('p_mw_bss_damped: ' + str(p_mw_bss[0]))
      
      # Limitation by maximum power:     
      p_mw_bss[bss.max_p_mw < p_mw_bss] = bss.max_p_mw[bss.max_p_mw < p_mw_bss]
      p_mw_bss[-bss.max_p_mw > p_mw_bss] = - bss.max_p_mw[-bss.max_p_mw > p_mw_bss]
      print('p_mw_bss limited: ' + str(p_mw_bss[0]))
      ...
      grid.net.storage = bss
      print('p_mw_bss end: ' + str(bss.p_mw[0]))
      print('p_mw_bss end sum: ' + str(bss.p_mw.sum()))
