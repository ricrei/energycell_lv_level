...
class BSScontroller:
    def __init__(self, grid, control):
        ...
        self.efficiency_charge = (
            grid.net.storage.efficiency_AC2Bat
            * grid.net.storage.efficiency_storage ** 0.5
        )
        self.efficiency_discharge = (
            grid.net.storage.efficiency_Bat2AC
            * grid.net.storage.efficiency_storage ** 0.5
        )
        ...
        if self.set_bss == True:
            if self.control == "direct":
                self.P_controller = BSS_control_direct(
                    grid,
                ...
                self.P_controller = BSS_P_control_hh_fid(
                    grid,
                    self.intervall_in_seconds,
                    self.busses_num,
                    self.timedelta_charging_delay,
                ...
                self.P_controller = BSS_P_control_grid_fid(
                    grid,
                    self.intervall_in_seconds,
                    self.busses_num,
                    self.timedelta_charging_delay,
            ...
            self.P_controller = BSS_control_no_bss(
                grid,
                self.intervall_in_seconds,
                self.busses_num,
                self.timedelta_charging_delay,
...
class BSS_control:
    def __init__(
        self,
        grid,
        intervall_in_seconds,
        bss_num,
        timedelta_charging_delay,
        timedelta_charging_delay_winter,
        timedelta_early_discharge,
        efficiency_charge,
    ...
    def residual_load_per_bus(self, grid):
        ...
        """
        residual_load_per_bus = (
            grid.net.load.loc[grid.load_index, "p_mw"].values
    ...
    def direct_charge(self, grid, p_mw_bss):
        ...
        solar_excess_2 = (p_mw_bss > 0) & (get_soc >= 100)
        ...
        load_excess_2 = (p_mw_bss < 0) & (get_soc <= 0)

        # Excess in solar power:
        p_mw_bss[solar_excess_1] = (
            free_capacity[solar_excess_1]
            * 3600
            / (self.intervall * self.efficiency_charge[solar_excess_1])
        )
        p_mw_bss[solar_excess_2] = 0

        # Excess in load:
        p_mw_bss[load_excess_1] = (
            -get_e_mwh[load_excess_1]
    ...
    def linear_charge(
        ...
        (
            ...
            sunset,
            timedelta_day_s,
            timedelta_sunrise_sunset_s,
        ) = grid.get_timedelta(t)
        (
            sunrise_next,
            sunset_next,
            timedelta_day_next_s,
            timedelta_sunrise_sunset_next_s,
        ) = grid.get_timedelta(t + datetime.timedelta(days=1))

        timedelta_charging_delay_winter_h = (
            timedelta_charging_delay_winter * timedelta_sunrise_sunset_s / 3600
        )  # [h]
        timedelta_charging_delay_summer_h = (
            timedelta_charging_delay * timedelta_sunrise_sunset_s / 3600
        )  # [h]
        timedelta_early_discharge = (
            timedelta_early_discharge * timedelta_sunrise_sunset_s / 3600
        )  # [h]

        t_start_linear_charge_summer = sunrise + datetime.timedelta(
            hours=timedelta_charging_delay_summer_h
        )
        t_start_linear_charge_winter = sunrise + datetime.timedelta(
            hours=timedelta_charging_delay_winter_h
        )
        t_end_linear_charge_summer = sunset - datetime.timedelta(
            hours=timedelta_charging_delay_summer_h
        )
        t_end_linear_charge_winter = sunset - datetime.timedelta(
            hours=timedelta_charging_delay_winter_h
        )

        t_start_linear_discharge = sunset - datetime.timedelta(
            hours=self.timedelta_early_discharge
        )
        timedelta_lin_ch_summer_s = abs(
            (t_end_linear_charge_summer - time).total_seconds()
        )
        timedelta_lin_ch_winter_s = abs(
            (t_end_linear_charge_winter - time).total_seconds()
        )
        timedelta_lin_dch_day1_s = abs((sunrise_next - time).total_seconds())
        timedelta_lin_dch_day2_s = abs((sunrise - time).total_seconds())
        timedelta_lin_dch_winter_day_s = abs(
            (t_start_linear_discharge - time).total_seconds()
        )
        ...
        if timedelta_sunrise_sunset_s < (12 * 3600):
            ...
            # charge:
            if (
                time >= t_start_linear_charge_winter
                and time <= t_end_linear_charge_winter
            ):
                p_mw_lin_ch = (free_capacity * 3600) / (
                    timedelta_lin_ch_winter_s * self.efficiency_charge
                )
            else:
            ...
            p_mw_lin_dch = (
                -(
                    (get_e_mwh - soc_reserve * grid.net.storage.max_e_mwh)
                    * self.efficiency_discharge
                    * 3600
                )
                / timedelta_night_s
            ...
            case_lin_dch = (
                (p_mw_lin_dch > p_mw_bss)
                & (p_mw_bss < 0)
                & (get_e_mwh >= soc_reserve * grid.net.storage.max_e_mwh)
            )
            case_lin_dch2 = (p_mw_bss < 0) & (
                get_e_mwh < soc_reserve * grid.net.storage.max_e_mwh
            )
        ...
        # Summer - Temporal parameters
            ...
            if (
                time >= t_start_linear_charge_summer
                and time <= t_end_linear_charge_summer
            ):
                p_mw_lin_ch = (free_capacity * 3600) / (
                    timedelta_lin_ch_summer_s * self.efficiency_charge
                )
            else:
                p_mw_lin_ch = np.zeros(len(grid.net.storage))
    ...
    def pcontrol_trafo_charge(self, grid, t):
        ...
        if grid.s_trafo_power < -s_res:  # trafo overloading, solar excess
            ...
            if q_res_to_the_power_of_2 < grid.s_trafo_power ** 2:
                p_trafo_max = (grid.s_trafo_power ** 2 - q_res_to_the_power_of_2) ** (
                    0.5
                )
            else:
                p_trafo_max = 0
            ...
            p_mw_damped[solar_excess_fid_2] = (
                free_capacity[solar_excess_fid_2]
                * 3600
            ...
            p_mw_damped[load_excess_fid_2] = -(
                get_e_mwh[load_excess_fid_2]
                * 3600
                * self.efficiency_discharge[load_excess_fid_2]
                / self.intervall
            )
