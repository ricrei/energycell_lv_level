...
def shorted_data(data, t_freq):
    ...
    data_shorted.index = pd.DatetimeIndex(
        data_shorted.index, tz=None, ambiguous='infer')
...
def plot_generation_consumption_as_heat_map_overall_eva(power, save_fig_dir=None):
    ...
    power_pivot = power.pivot_table(
        columns=power.index.dayofyear, index=power.index.hour + power.index.minute/60)

    plt.figure(figsize=(10, 8))
    ax = sns.heatmap(power_pivot.pv - power_pivot.load -
                     power_pivot.hp - power_pivot.ev, center=0)
...
def plot_residualload_overall_eva(power, save_fig_dir=None):
    ...
    ax.fill_between(power.index, -power.load-power.ev, -
                    power.hp-power.load-power.ev, alpha=0.7)
    ax.plot(power.index, -power.hp-power.load-power.ev, lw=.6)
    #power = shorted_data(power, 'W')
    ax.plot(power.index, power.pv-power.hp -
            power.load-power.ev, color='black', lw=1)
    ax.set_xlabel('Time')
    ...
    plt.legend(['Photovoltaic generation', 'E-vehicle load', 'Household load',
                'Heat pump load', 'Residual Load'], loc='lower right')
    if save_fig_dir is not None:
        plt.savefig(save_fig_dir, bbox_inches='tight')
...
def plot_residualload_subplot_overall_eva(eva1, eva2, save_fig_dir=None):
    ...
    fig, ax = plt.subplots(1, 2, figsize=(
        20, 5), sharey=True,  gridspec_kw={'wspace': .05})
    for i in [0, 1]:
        ...
        ax[i].fill_between(power[i].index,                     -storage_discharge,
                           -storage_discharge - power[i].pv, alpha=0.7, color=c[0])
        ax[i].fill_between(power[i].index,                         storage_charge,
                           power[i].ev + storage_charge, alpha=0.7, color=c[1])
        ax[i].fill_between(power[i].index,              power[i].ev + storage_charge,
                           power[i].load + power[i].ev + storage_charge, alpha=0.7, color=c[2])
        ax[i].fill_between(power[i].index, power[i].load + power[i].ev + storage_charge,
                           power[i].hp + power[i].load + power[i].ev + storage_charge, alpha=0.7, color=c[3])
        ax[i].fill_between(power[i].index, 0, storage_charge,
                           alpha=0.7, color=c[4])
        ax[i].fill_between(power[i].index, 0, -
                           storage_discharge, alpha=0.7, color=c[4])
        # curtailed power
        ax[i].fill_between(power[i].index, power[i].hp + power[i].load + power[i].ev + storage_charge, power[i].hp +
                           power[i].load + power[i].ev + storage_charge + curtailed[i].curtail_load, alpha=0.2, color=c[7])
        ax[i].fill_between(power[i].index,                     -storage_discharge - power[i].pv,
                           -storage_discharge - power[i].pv - curtailed[i].curtail_pv, alpha=0.2, color=c[7])

        ax[i].plot(power[i].index, -storage_discharge-power[i].pv, lw=.6)
        ax[i].plot(power[i].index, storage_charge+power[i].ev, lw=.6)
        ax[i].plot(power[i].index, storage_charge +
                   power[i].load+power[i].ev, lw=.6)
        ax[i].plot(power[i].index, storage_charge +
                   power[i].hp+power[i].load+power[i].ev, lw=.6)
        ...
        ax[i].plot(power[i].index, -(power[i].pv-power[i].hp -
                                     power[i].load-power[i].ev+storage_sum), color='black', lw=1)

        ax[i].set_xticks([k for k in power[i].index if (k.hour == 12) & (
            k.minute == 0)])  # auskommentieren für frößeren Zeitraum
        # auskommentieren für frößeren Zeitraum
    ...
    plt.rcParams['font.size'] = '16' #size of labels on axis
  ...
  '''
    ...
    time_min_summer = pd.to_datetime('2017-05-26 00:00:00+02:00', utc=True)
  ...
  time_max_1 = pd.to_datetime('2017-01-09 00:00:00+01:00', utc=True)
...
def plot_residualload_subplot_overall_eva_no_bss(eva1, eva2, save_fig_dir=None):
    ...
    for i in [0, 1]:
        ...
        ax[i].fill_between(power[i].index,              power[i].ev,
                           power[i].load + power[i].ev, alpha=0.7, color=c[2])
        ax[i].fill_between(power[i].index, power[i].load + power[i].ev,
                           power[i].hp + power[i].load + power[i].ev, alpha=0.7, color=c[3])
        ax[i].fill_between(power[i].index, 0, power[i].ev,
                           alpha=0.7, color=c[1])
        ax[i].fill_between(power[i].index, 0, -
                           power[i].pv, alpha=0.7, color=c[0])
        # curtailed power
        ax[i].fill_between(power[i].index, power[i].hp + power[i].load + power[i].ev, power[i].hp +
                           power[i].load + power[i].ev + curtailed[i].curtail_load, alpha=0.2, color=c[7])
        ax[i].fill_between(power[i].index,                      - power[i].pv,
                           - power[i].pv - curtailed[i].curtail_pv, alpha=0.2, color=c[0])
        ...
        ax[i].plot(power[i].index, -(power[i].pv-power[i].hp -
                                     power[i].load-power[i].ev), color='black', lw=1)
...
def plot_residualload_subplot_overall_eva_single_case(eva1, eva2, save_fig_dir=None):
    ...
    storage_sum = storage_1.sum(axis=1)
    ...
    ax.fill_between(power.index,                     -storage_discharge,
                    -storage_discharge - power.pv, alpha=0.7, color=c[0])
    ax.fill_between(power.index,                         storage_charge,
                    power.ev + storage_charge, alpha=0.7, color=c[1])
    ax.fill_between(power.index,              power.ev + storage_charge,
                    power.load + power.ev + storage_charge, alpha=0.7, color=c[2])
    ax.fill_between(power.index, power.load + power.ev + storage_charge,
                    power.hp + power.load + power.ev + storage_charge, alpha=0.7, color=c[3])
    ...
    # curtailed power
    ax.fill_between(power.index, power.hp + power.load + power.ev + storage_charge, power.hp +
                    power.load + power.ev + storage_charge + curtailed.curtail_load, alpha=0.2, color=c[7])
    ax.fill_between(power.index,                     -storage_discharge - power.pv,
                    -storage_discharge - power.pv - curtailed.curtail_pv, alpha=0.2, color=c[7])
    ...
    ax.plot(power.index, -(power.pv-power.hp-power.load -
                           power.ev+storage_sum), color='black', lw=1)
    ...
    ax.set(xlim=(power.index[0], power.index[-1]), ylim=(-1.100, .850))
    patch_list = [
        mpatches.Patch(color=c[0], alpha=0.7, label='Photovoltaik'),
        mpatches.Patch(color=c[1], alpha=0.7, label='E-Auto'),
        mpatches.Patch(color=c[2], alpha=0.7, label='Haushalt'),
    ...
    ax.set_ylabel('Power in MW')
    #ax.set_xlabel('Without BSS')
    ax.set_xlabel('With BSS')
...
def plot_residualload_subplot_overall_eva_timeslot(power_summer, power_winter, save_fig_dir=None):
    ...
    for i in [0, 1]:
        ...
        ax[i].plot(power[i].index, -power[i].ev, lw=.6)
        ax[i].fill_between(power[i].index, -power[i].ev, -
                           power[i].load-power[i].ev, alpha=0.7)
        ax[i].plot(power[i].index, -power[i].load-power[i].ev, lw=.6)
        ax[i].fill_between(power[i].index, -power[i].load-power[i].ev, -
                           power[i].hp-power[i].load-power[i].ev, alpha=0.7)
        ax[i].plot(power[i].index, -power[i].hp -
                   power[i].load-power[i].ev, lw=.6)
        power[i] = shorted_data(power[i], '10T')
        ax[i].plot(power[i].index, power[i].pv-power[i].hp -
                   power[i].load-power[i].ev, color='black', lw=1)
        ax[i].set_xticks(
            [k for k in power[i].index if (k.hour == 12) & (k.minute == 0)])
        ax[i].set_xticklabels(
            ['  Day 1', '  Day 2', '  Day 3', '  Day 4', '  Day 5', '  Day 6', '  Day 7'])
        ax[i].set(xlim=(power[i].index[0], power[i].index[-1]),
                  ylim=(-2.000, .500))
        plt.legend(['PV Generation', 'EV Load', 'Household Load', 'HP Load',
                    'Residual Load'], loc='lower right', shadow=True, markerscale=1.0)
    ...
    time_max_summer = pd.to_datetime('2017-05-30 00:00:00+01:00', utc=True)
    #time_max_summer = pd.to_datetime('2017-05-29 00:00:00+01:00', utc=True)
...
def plot_violin_overall_eva(df_eva_v, df_eva_l, save_fig_dir=None):
    ...
    plt.figure()
    ax = sns.violinplot(x="gridID", y="voltage", hue='timescope',
                        data=df_eva_v, palette="muted", split=True, inner="quartile", cut=0)

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'violin_voltage.png', bbox_inches='tight')

    plt.figure()
    ax = sns.violinplot(x="gridID", y="lineloading", hue='timescope',
                        data=df_eva_l, palette="muted", split=True, inner="quartile", cut=0)
...
def plot_barplots_overall_eva(eva, n, save_fig_dir=None):
    ...
    df_bar = pd.DataFrame(index=range(
        len(eva.keys())*4), columns=['type', 'value', 'scenario', 'gridID', 'timescope'])
    ...
    df_bar.loc[df_bar['type'] == 'v_over',
               'value'] = df_bar.loc[df_bar['type'] == 'v_over', 'value']/v_events
    df_bar.loc[df_bar['type'] == 'v_under',
               'value'] = df_bar.loc[df_bar['type'] == 'v_under', 'value']/v_events
    df_bar.loc[df_bar['type'] == 'll_over',
               'value'] = df_bar.loc[df_bar['type'] == 'll_over', 'value']/l_events
    df_bar.loc[df_bar['type'] == 'tl_over',
               'value'] = df_bar.loc[df_bar['type'] == 'tl_over', 'value']/t_events
...
def plot_heatmap_grid_issus(eva, net_name, n, save_fig_dir=None):
    ...
    df_v_heatmap = pd.DataFrame(index=[net_name[7:12]], columns=[
                                100, 200, 300, 400]).fillna(0)
    df_l_heatmap = pd.DataFrame(index=[net_name[7:12]], columns=[
                                100, 200, 300, 400]).fillna(0)
    df_t_heatmap = pd.DataFrame(index=[net_name[7:12]], columns=[
                                100, 200, 300, 400]).fillna(0)
    v_events = pd.DataFrame(index=[net_name[7:12]], columns=[
                            100, 200, 300, 400]).fillna(0)
    l_events = pd.DataFrame(index=[net_name[7:12]], columns=[
                            100, 200, 300, 400]).fillna(0)
    t_events = pd.DataFrame(index=[net_name[7:12]], columns=[
                            100, 200, 300, 400]).fillna(0)

    for index in eva:
        if eva[index]['scenario'] in [100, 200, 300, 400]:
            df_v_heatmap[eva[index]['scenario']].loc[eva[index]['net_name']
                                                     ] += (eva[index]['v_under'] + eva[index]['v_over'])/n
            df_l_heatmap[eva[index]['scenario']].loc[eva[index]
                                                     ['net_name']] += eva[index]['ll_over']/n
            df_t_heatmap[eva[index]['scenario']].loc[eva[index]
                                                     ['net_name']] += eva[index]['tl_over']/n
            v_events[eva[index]['scenario']].loc[eva[index]
                                                 ['net_name']] += len(eva[index]['v'].columns)/n
            l_events[eva[index]['scenario']].loc[eva[index]
                                                 ['net_name']] += len(eva[index]['ll'].columns)/n
            t_events[eva[index]['scenario']].loc[eva[index]
                                                 ['net_name']] += len(eva[index]['tl'].columns)/n
    ...
    plt.figure()
    ax = sns.heatmap(df_v_heatmap/v_events/minutes_per_week*100, vmax=vmax,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    ...
    plt.figure()
    ax = sns.heatmap(df_l_heatmap/l_events/minutes_per_week*100, vmax=vmax,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set_xticklabels(x_ticklabels)
    ax.set_yticklabels(y_ticklabels)
    ...
    plt.figure()
    ax = sns.heatmap(df_t_heatmap/t_events/minutes_per_week*100, vmax=vmax,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set_xticklabels(x_ticklabels)
    ax.set_yticklabels(y_ticklabels)
...
def plot_heatmap_grid_issus_with_bss(eva, net_name, n, save_fig_dir=None):
    ...
    df_v_heatmap = pd.DataFrame(index=[net_name[7:12]], columns=[
                                400, 600, 610, 620, 630, 640]).fillna(0)
    df_l_heatmap = pd.DataFrame(index=[net_name[7:12]], columns=[
                                400, 600, 610, 620, 630, 640]).fillna(0)
    df_t_heatmap = pd.DataFrame(index=[net_name[7:12]], columns=[
                                400, 600, 610, 620, 630, 640]).fillna(0)
    ...
    ax = sns.heatmap(df_v_heatmap/v_events/minutes_per_week*100, vmax=vmax,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    #ax.set(xlabel='Grid', ylabel='Scenario')
    ax.set_xticklabels(x_ticklabels)
    ax.set_yticklabels(y_ticklabels)
    #ax.set_title('Voltage Violation in %')
    ax.set_title('Spannungsbandverletzung [%]')
    #ax.set_title('Voltage Violation in Minutes per Week and Bus')

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_voltage_bss.pdf',
                    bbox_inches='tight')
    ...
    ax.set_yticklabels(y_ticklabels)
    #ax.set_title('Line Overloading in %')
    ax.set_title('Leitungsüberlastung [%]')
    #ax.set_title('Line Overloading in Minutes per Week and Line')

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_lineloading_bss.pdf',
                    bbox_inches='tight')
    ...
    ax.set_yticklabels(y_ticklabels)
    #ax.set_title('Trafo Overloading in %')
    ax.set_title('Transformatorüberlastung [%]')
    #ax.set_title('Line Overloading in Minutes per Week and Line')

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_trafoloading_bss.pdf',
                    bbox_inches='tight')
...
def plot_heatmap_grid_issus_with_bss_curtailed(eva, net_name, n, save_fig_dir=None):
    ...
    if save_fig_dir is not None:
        plt.savefig(save_fig_dir +
                    'heatmap_voltage_bss_curtailed.pdf', bbox_inches='tight')
    ...
    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + 'heatmap_lineloading_bss_curtailed.pdf', bbox_inches='tight')
    ...
    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + 'heatmap_trafoloading_bss_curtailed.pdf', bbox_inches='tight')
...
def plot_heatmap_curtailed_power(eva, net_name, n, save_fig_dir=None):
   ...
   # print(eva) 
    ...
    for index in eva:
        ...
        if eva[index]['scenario'] in [401, 601, 611, 621, 631, 641]:  
            ...
            curtailed_power_load[eva[index]['scenario']].loc[eva[index]
                                                             ['net_name']] += eva[index]['curtailed_power'].curtail_load.sum()
            pv_power[eva[index]['scenario']].loc[eva[index]
                                                 ['net_name']] += eva[index]['power'].pv.sum()
            load_power[eva[index]['scenario']].loc[eva[index]['net_name']] += eva[index]['power'].load.sum(
            ) + eva[index]['power'].hp.sum() + eva[index]['power'].ev.sum()
            if eva[index]['time_scope_name'] == 'summer':
                curtailed_power_summer[eva[index]['scenario']].loc[eva[index]
                                                                   ['net_name']] += eva[index]['curtailed_power'].curtail_pv.sum()
                # print('summer')
            elif eva[index]['time_scope_name'] == 'winter':
                curtailed_power_winter[eva[index]['scenario']].loc[eva[index]
                                                                   ['net_name']] += eva[index]['curtailed_power'].curtail_pv.sum()
                # print('winter')

    curtailed_power_pv_per_cent = curtailed_power_pv / \
    ...
    vmax_pv_load_per_cent = np.array(
        [vmax_pv_per_cent, vmax_load_per_cent]).max()

    vmin = 0

    plt.figure()
    ax = sns.heatmap(curtailed_power_summer/1000, vmax=vmax_season,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ax.set_xticklabels(x_ticklabels)
    ...
    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + '00a_heatmap_curtailed_power_summer.pdf', bbox_inches='tight')

    # "YlOrBr"

    plt.figure()
    ax = sns.heatmap(curtailed_power_winter/1000, vmax=vmax_season,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ax.set_xticklabels(x_ticklabels)
    ...
    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + '00a_heatmap_curtailed_power_winter.pdf', bbox_inches='tight')

    plt.figure()
    ax = sns.heatmap(curtailed_power_pv_per_cent, vmax=vmax_pv_load_per_cent,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ax.set_xticklabels(x_ticklabels)
    ...
    ax.set_title('Abgeregelte Energie (Photovoltaik) [%]')
    #ax.set_title('Voltage Violation in Minutes per Week and Bus')

    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + '00b_heatmap_curtailed_power_pv_per_cent.pdf', bbox_inches='tight')
        #plt.savefig(save_fig_dir + '00b_heatmap_curtailed_power_pv_per_cent_' + str(eva[index]['time_scope_name']) + '.png', bbox_inches='tight')

    # "YlOrBr"

    plt.figure()
    ax = sns.heatmap(curtailed_power_load_per_cent, vmax=vmax_pv_load_per_cent,
                     vmin=vmin, cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ax.set_xticklabels(x_ticklabels)
    ...
    ax.set_title('Abgeregelte Energie (Verbrauch) [%]')
    #ax.set_title('Line Overloading in Minutes per Week and Line')

    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + '00b_heatmap_curtailed_power_load_per_cent.pdf', bbox_inches='tight')
        #plt.savefig(save_fig_dir + '00b_heatmap_curtailed_power_load_per_cent_' + str(eva[index]['time_scope_name']) + '.png', bbox_inches='tight')

    plt.figure()
    ax = sns.heatmap(curtailed_power_pv/1000, vmax=vmax_pv_load, vmin=vmin,
                     cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ax.set_xticklabels(x_ticklabels)
    ax.set_yticklabels(y_ticklabels)
    ...
    if save_fig_dir is not None:
        plt.savefig(save_fig_dir +
                    '00c_heatmap_curtailed_power_pv.pdf', bbox_inches='tight')
        #plt.savefig(save_fig_dir + '00c_heatmap_curtailed_power_pv_' + str(eva[index]['time_scope_name']) + '.png', bbox_inches='tight')

    # "YlOrBr"

    plt.figure()
    ax = sns.heatmap(curtailed_power_load/1000, vmax=vmax_pv_load, vmin=vmin,
                     cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ax.set_xticklabels(x_ticklabels)
    ...
    if save_fig_dir is not None:
        plt.savefig(
            save_fig_dir + '00c_heatmap_curtailed_power_load.pdf', bbox_inches='tight')
        #plt.savefig(save_fig_dir + '00c_heatmap_curtailed_power_load_' + str(eva[index]['time_scope_name']) + '.png', bbox_inches='tight')


def plot_heatmap_self_sufficiency_curtailed(eva, net_name, n, save_fig_dir=None):
    print('Create Heatmap plots curtailed power')
    ...
    df_self_sufficiency = pd.DataFrame(index=[net_name[7:12]], columns=[
                                       401, 601, 611, 621, 631, 641]).fillna(0)
    df_pv_consumption = pd.DataFrame(index=[net_name[7:12]], columns=[
                                     401, 601, 611, 621, 631, 641]).fillna(0)
    ...
    plt.figure()
    ax = sns.heatmap(df_self_sufficiency, vmax=vmax, vmin=vmin,
                     cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    #ax.set(xlabel='Grid', ylabel='Scenario')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ...
    ax.set_title('Autarkiegrad [%]')
    #ax.set_title('Voltage Violation in Minutes per Week and Bus')

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_self_sufficiency.pdf',
                    bbox_inches='tight')

    plt.figure()
    ax = sns.heatmap(df_pv_consumption, vmax=vmax, vmin=vmin,
                     cmap="rocket_r", annot=True, square=True)  # , fmt=".0f")
    #ax.set(xlabel='Scenario', ylabel='Grid')
    #ax.set(xlabel='Grid', ylabel='Scenario')
    ax.set(xlabel='Szenario', ylabel='Energiezelle')
    ...
    ax.set_title('Eigenverbrauchsanteil [%]')

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_pv_consumption.pdf',
                    bbox_inches='tight')
...
def plot_heatmap_self_sufficiency(eva, net_name, n, save_fig_dir=None):
    ...
    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_self_sufficiency_not_curtailed.pdf',
                    bbox_inches='tight')
    ...
    if save_fig_dir is not None:
        plt.savefig(save_fig_dir + 'heatmap_pv_consumption_not_curtailed.pdf',
                    bbox_inches='tight')
...
def plot_grid_issus_over_power(eva, save_fig_dir=None):
    ...
    plt.legend(handles=[line1, line2, line3, line4], labels=[
               'overvoltage', 'undervoltage', 'line overload', 'trafo overload'])
...
def plot_grid_issus_over_time(eva, save_fig_dir=None):
    ...
    ax1.legend(handles=[l2, l1, l_limit], labels=[
               'Max Voltage', 'Min Voltage', 'Voltage Limit'], loc="upper right")
    ax2.legend(handles=[l4, l3, l_limit2], labels=[
               'Lineloading', 'Trafoloading', 'Overload Limit'], loc="upper right")
    ax3.legend(handles=[l5], labels=['Residualload'], loc="lower right")
    ax1.set_xlabel('Time')
    ax1.set_ylabel('Voltage\nin p.u.')
    ax2.set_ylabel('Line and Trafo\nLoading in p.u.')
    ax3.set_ylabel('Power\nin p.u.')
    ax1.set_xticks([i for i in power.index if (
        i.hour == 12) & (i.minute == 0)])
    #ax1.set_xticklabels(['  Day 1', '  Day 2', '  Day 3',
                         #'  Day 4', '  Day 5', '  Day 6', '  Day 7'])
    ax1.set_xticklabels(['  Tag 1', '  Tag 2', '  Tag 3',
                         '  Tag 4', '  Tag 5', '  Tag 6', '  Tag 7'])
    ax2.set_xticks([i for i in power.index if (
        i.hour == 12) & (i.minute == 0)])
    #ax2.set_xticklabels(['  Day 1', '  Day 2', '  Day 3',
                         #'  Day 4', '  Day 5', '  Day 6', '  Day 7'])
    ax2.set_xticklabels(['  Tag1', '  Tag 2', '  Tag 3',
                         '  Tag 4', '  Tag 5', '  Tag 6', '  Tag 7'])
    ax3.set_xticks([i for i in power.index if (
        i.hour == 12) & (i.minute == 0)])
    #ax3.set_xticklabels(['  Day 1', '  Day 2', '  Day 3',
                         #'  Day 4', '  Day 5', '  Day 6', '  Day 7'])
    ax3.set_xticklabels(['  Tag 1', '  Tag 2', '  Tag 3',
                         '  Tag 4', '  Tag 5', '  Tag 6', '  Tag 7'])
...
def plot_grid_issus_over_time_subplot(eva_summer, eva_winter, detailed=None, save_fig_dir=None):
    ...
    for i in [0, 1]:
        ...
        ax3[i].set_xticklabels(
            ...
            #['  Day 1', '  Day 2', '  Day 3', '  Day 4', '  Day 5', '  Day 6', '  Day 7'])
        if detailed == None:
            ax1[i].set(xlim=(power[i].index[0], power[i].index[-1]),
                       ylim=(.85, 1.15))
            ax2[i].set(xlim=(power[i].index[0],
                             power[i].index[-1]), ylim=(0, 7.0))
            ax3[i].set(xlim=(power[i].index[0], power[i].index[-1]),
                       ylim=(-2.2, 1))
            '''
            ax1[1].legend(handles=[l2, l1, l_limit], labels=[
                          'Max Voltage', 'Min Voltage', 'Voltage Limit'], loc="upper right", shadow=True)
            ax2[1].legend(handles=[l4, l3, l_limit2], labels=[
                          'Lineloading', 'Trafoloading', 'Overload Limit'], loc="upper right", shadow=True)
            ax3[1].legend(handles=[l5], labels=['Residualload'],
            ...
            '''
            ax1[1].legend(handles=[l2, l1, l_limit], labels=[
                          'Max Voltage', 'Min Voltage', 'Voltage Limit'], loc="upper left", shadow=True)
            ax2[1].legend(handles=[l4, l3, l_limit2], labels=[
                          'Lineloading', 'Trafoloading', 'Overload Limit'], loc="upper left", shadow=True)
            ax3[1].legend(handles=[l5], labels=['Residualload'],
...
H is the hatch used for identification of the different dataframe"""
        ...
        for j, pa in enumerate(h[i:i+n_col]):
            for rect in pa.patches:  # for each index
                rect.set_x(rect.get_x() + 1 / float(n_df + 1)
                           * i / float(n_col))
                # (H * int(2*i / n_col)) #edited part
                rect.set_hatch(H * int(3*i / n_col))
                rect.set_width(1 / float(n_df + 1))
                rect.set_alpha(.9)
                rect.set_aa(True)

                axe.set_xticks(
                    (np.arange(0, 2 * n_ind, 2) + .5 / float(n_df + 1)) / 2.)
    ...
    if labels1 is not None:
        # (h[:n_col], l[:n_col], loc=[1.01, 0.5])
        l1 = axe.legend(h[:n_col], labels1, loc=[.01, .82])
...
def plot_curtailed_power(eva, scenario, save_fig_dir=None):
    ...
    for index in eva:
        if eva[index]['scenario'] == scenario:
            ...
            df['feed-in_power_pv'].iloc[i] = eva[index]['power']['pv'].sum() * \
                (100-eva[index]['PVConsumption'])/100*scale_factor_energy
            df['self-consumed_power_pv'].iloc[i] = eva[index]['power']['pv'].sum() * \
                (eva[index]['PVConsumption'])/100*scale_factor_energy
            df['curtailed_power_pv'].iloc[i] = curtailed_power['curtail_pv'] * \
                scale_factor_energy
    ...
    plot_clustered_stacked([df_winter_pv, df_summer_pv], [
                           'curtailed', 'feed-in', 'self-consumed'], ['winter', 'summer'], title=' ')

    if save_fig_dir is not None:
        plt.savefig(save_fig_dir+'curtailed_PV_power_' +
                    str(scenario)+'.png', bbox_inches='tight')
    else:
        plt.show()

    plot_clustered_stacked([df_winter_load, df_summer_load], [
                           'curtailed', 'grid-obtained', 'self-consumed'], ['winter', 'summer'], title=' ')
...
def state_of_charge(eva_summer, eva_winter, save_fig_dir=None):
    ...
    soc = [soc_summer, soc_winter]
    
    fig, ax = plt.subplots(1, 2, figsize=(
        20, 5), sharey=True,  gridspec_kw={'wspace': .05})
    for i in [0, 1]:
        ax[i].plot(soc[i].index, soc[i], lw=1)
        ax[i].plot(soc[i].index, soc[i].mean(axis=1), color='black', linewidth = '2')

        ax[i].set_xticks([k for k in soc[i].index if (k.hour == 12) & (
            k.minute == 0)])  # auskommentieren für frößeren Zeitraum
        # auskommentieren für frößeren Zeitraum
        #ax[i].set_xticklabels(
            #['  Day 1', '  Day 2', '  Day 3', '  Day 4', '  Day 5', '  Day 6', '  Day 7']) 
  ...
  '''
    ...
    #---------------------------------------
    fig, ax = plt.subplots()
    ax.plot(soc_winter.index, soc_winter)
    ax.plot(soc_winter.index, soc_winter.mean(axis=1), color='black', linewidth = '2')
...
def radar_chart_scenario_comparisson(eva, net_name, n, save_fig_dir=None):
   ...
   # print(df_self_sufficiency)
    # print(df_self_sufficiency[eva[index]['scenario']])#.loc[eva[index]['net_name']])
    # print(df_self_sufficiency[eva[index]['net_name']])
    # print(df_self_sufficiency['net_name'])
    '''
...
def test(eva, net_name, n, save_fig_dir=None):
    categories = ['processing cost','mechanical properties','chemical stability',
              'thermal stability']
