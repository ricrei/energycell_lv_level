...
class MainEconomics():
  ...
  def energyflow_on_HH_level(self):
    ...
    bss_pos = pd.DataFrame(index=bss.index, columns=bss.columns).fillna(0)
    bss_pos[bss>0] = bss[bss>0] # flex durch last
    ...
    Ec_self[dE_HH >= 0] = pv[dE_HH >= 0] - bss[dE_HH >= 0]# + flex_bss[dE_HH >= 0]
    Eg_self[dE_HH >= 0] = pv[dE_HH >= 0] - bss[dE_HH >= 0]# + flex_bss[dE_HH >= 0]
    ...
    for i in ['7']:#Ec_LV.columns:#
      #ax.plot(dE_HH[i])
      ax.plot(Eg_self[i])
      #ax.plot(Eg_self[i] + Eg_self_flex[i])
      ax.plot(Eg_LV[i])
      ax.plot(Eg_MV[i])
      #ax.plot(load[i])
      #ax.plot(bss[i])
    ...
    '''
    fig, ax = plt.subplots()
    ax.plot(self.trafo_p)
    ax.plot(Ec_MV.sum(axis=1))
    ax.plot(-Eg_MV.sum(axis=1))
    #ax.plot(load[i])
    #ax.plot((abs(Ec_self) + abs(Ec_MV) + abs(Ec_LV) - (load)))
    #ax.plot((abs(Eg_self) + abs(Eg_MV) + abs(Eg_LV) - (pv - bss)))
    ax.set_xlabel('Time')
    ...
    # Bilanzcheck
    print_mmm((abs(Ec_self) + abs(Ec_MV) + abs(Ec_LV) + abs(Ec_self_flex) + abs(Ec_LV_flex) - (load)))
    print_mmm((abs(Eg_self) + abs(Eg_MV) + abs(Eg_LV) + abs(Eg_self_flex) + abs(Eg_LV_flex) - (pv - bss)))
