...
def new_scenario(start = '2017-01-01 00:01:00', 
                 end = '2017-12-31 23:59:00', 
                 periods = None, freq = "15 min", column = 'Demand'):

    df_main = pd.DataFrame(pd.date_range(start, end, periods, freq, name ='TIMESTAMP'))
    df_main[column] = 0
...
def cop(mean_temp_hours, heatpump_type = "Air", water_temp = 60):
    ...
    if heatpump_type == "Air":
        for tmp in mean_temp_hours:#.iterrows():
...
def hp_loadshape(building_type, SigLinDe, mean_temp_days, t_0, demand_daily, mean_temp_hours, heatpump_type, water_temp, hours_year, heatpump_power, df):
    ...
    h_del_para = h_del(mean_temp_days, b_params, t_0)

    heat_demand_daily = daily_demand(h_del_para, mean_temp_days.Mean_Temp, demand_daily)
    
    Q_N = demandfactor(hours_year, heatpump_power)
    
    K_w = consumerfactor(Q_N, h_del_para)

    heat_demand_h = hourly_heat_demand(heat_demand_daily, K_w)

    df_cop = cop(mean_temp_hours.ta, heatpump_type, water_temp)

    el_demand_h = hourly_el_demand(heat_demand_h, df_cop)
    el_demand_h.dropna(inplace = True)

    df_h = new_scenario(freq = "H")
    df_h["Demand_el"] = el_demand_h
    df_h["Demand_th"] = heat_demand_h
    df_h = df_h.set_index('TIMESTAMP')
    temp = df_h.loc['2017-12-31 23:01:00']

    new_row = {'Demand' : temp.Demand, 'Demand_el' : temp.Demand_el, 'Demand_th' : temp.Demand_th}
    new_row = pd.Series(new_row)
    df_h.loc[pd.to_datetime('2018-01-01 00:00:00', format='%Y-%m-%d %H:%M:%S'),:] = new_row

    df_h.dropna(inplace = True)
    df_min = df_h.resample('T').interpolate(method='linear')#.pad()
    del df_min['Demand']
